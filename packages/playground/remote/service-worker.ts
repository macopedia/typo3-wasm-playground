...
import { TYPO3RewriteRules } from '@typo3-playground/typo3';
 ...
 * like a 12MB typo3-static.zip file. We need to cache them these requests.
...
async function handleScopedRequest(event: FetchEvent, scope: string) {
	...
	if (
		...
		console.log(resolvedUrl.pathname);
...
type TYPO3ModuleDetails = {
	staticAssetsDirectory?: string;
};

const scopeToTYPO3Module: Record<string, TYPO3ModuleDetails> = {};
async function getScopedWpDetails(scope: string): Promise<TYPO3ModuleDetails> {
	if (!scopeToTYPO3Module[scope]) {
		const requestId = await broadcastMessageExpectReply(
			{
				method: 'getTYPO3ModuleDetails',
			},
			scope
		);
		scopeToTYPO3Module[scope] = await awaitReply(self, requestId);
	}
	return scopeToTYPO3Module[scope];
}
