...
import { getLoadedTYPO3Version } from '@typo3-playground/typo3';
 ...
 * a list is shipped with every minified build in a file called `typo3-remote-asset-paths`.
 *
 ...
 * `/wordpress/typo3-remote-asset-paths`and finds it there. This means it's available on the
 ...
 *
 * The backfilling only runs when a non-empty `typo3-remote-asset-paths` file
...
export async function backfillStaticFilesRemovedFromMinifiedBuild(php: PHP) {
	...
	try {
		...
		const staticAssetsUrl = await getTypo3StaticZipUrl(php);
		if (!staticAssetsUrl) {
			return;
		}
		...
		await unzipFile(
			php,
			new File([await response!.blob()], 'typo3-static.zip'),
			php.requestHandler!.documentRoot,
			false
		);
		...
		logger.warn('Failed to download TYPO3 assets', e);
	}
}
...
/**
 * Returns the URL of the typo3-static.zip file containing all the
 ...
 */
export async function getTypo3StaticZipUrl(php: PHP) {
	const t3Version = await getLoadedTYPO3Version(php.requestHandler!);
	const staticAssetsDirectory = t3VersionToStaticAssetsDirectory(t3Version);
	if (!staticAssetsDirectory) {
		return false;
	}
	return joinPaths('/', staticAssetsDirectory, 'typo3-static.zip');
}
