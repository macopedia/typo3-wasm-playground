...
import type { Blueprint } from '@typo3-playground/blueprints';
	...
	async function previewPr(prValue: string) {
		...
		if (response.status !== 200) {
			...
			if (error === 'invalid_pr_number') {
				...
				setError(
					`The PR ${prNumber} couldn't be previewed due to an unexpected error. Please try again later or fill an issue in the TYPO3 Playground repository.`
				);
