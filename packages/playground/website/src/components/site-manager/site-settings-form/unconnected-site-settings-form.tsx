...
type ConfigurableFields = Record<
	keyof SiteFormData & ('t3Version' | 'language' | 'multisite'),
	boolean
>;
...
export function UnconnectedSiteSettingsForm({
	...
	const { supportedTypo3Versions, latestTypo3Version } =
		useSupportedWordPressVersions();

	useEffect(() => {
		if (
			latestTypo3Version &&
			['', 'latest'].includes(getValues('t3Version'))
		) {
			setValue('t3Version', latestTypo3Version);
		}
	}, [latestTypo3Version, setValue, getValues]);
		...
		>
			...
			<VStack spacing={3} className={css.formSection}>
				<Controller
					...
					rules={{
						required: {
							message: 'TYPO3 Version is required',
							value: true,
						},
					}}
					disabled={!enabledFields.t3Version}
					render={({ field: { onChange, ...rest } }) => (
						<div>
							<SelectControl
								...
								label="TYPO3 Version"
								labelPosition="side"
								help={errors.t3Version?.message}
								className={classNames(css.addSiteInput, {
									[css.invalidInput]: !!errors.t3Version,
								})}
								options={
									/*
									...
									[
										...
										...Object.keys(
											...
											label: `${supportedTypo3Versions[version]}`,
											value: version,
										})),
									]
								}
