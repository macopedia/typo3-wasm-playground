...
import { Blueprint, StepDefinition } from '@typo3-playground/blueprints';
import { logger } from '@php-wasm/logger';
import { setupPostMessageRelay } from '@php-wasm/web';
import { startPlaygroundWeb } from '@typo3-playground/client';
import { PlaygroundClient } from '@typo3-playground/remote';
...
export function bootSiteClient(
	...
	return async (
		...
		let isTypo3Installed = false;
		if (mountDescriptor) {
			try {
				isTypo3Installed = await playgroundAvailableInOpfs(
		...
		try {
			playground = await startPlaygroundWeb({
				...
				mounts: mountDescriptor
					...
					: [],
				shouldInstallTypo3: !isTypo3Installed,
 ...
 *
 * @TODO: Create a shared package like @typo3-playground/typo3 for such utilities
 ...
 *
 * @TODO: Reuse the "isTypo3Installed" logic implemented in the boot protocol.
