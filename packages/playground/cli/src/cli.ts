...
import {
	...
	compileBlueprint,
	runBlueprintSteps,
} from '@typo3-playground/blueprints';
import { isValidTypo3Slug } from './is-valid-typo3-slug';
import { EmscriptenDownloadMonitor, ProgressTracker } from '@php-wasm/progress';
import { createNodeFsMountHandler, loadNodeRuntime } from '@php-wasm/node';
import { RecommendedPHPVersion, zipDirectory } from '@typo3-playground/common';
...
async function run() {
	...
	const yargsObject = await yargs(process.argv.slice(2))
		.usage('Usage: typo3-playground <command> [options]')
		.positional('command', {
		...
		.option('skipWordPressSetup', {
			describe:
				'Do not download, unzip, and install WordPress. Useful for mounting a pre-configured TYPO3 directory at /wordpress.',
			type: 'boolean',
			default: false,
		})
		.option('quiet', {
		...
		.check((args) => {
			if (args.t3 !== undefined && !isValidTypo3Slug(args.t3)) {
				try {
					// Check if is valid URL
					new URL(args.t3);
				} catch (e) {
					throw new Error(
						'Unrecognized TYPO3 version. Please use "latest", a URL, or a numeric version such as "13.4"'
					);
				}
			}
		...
		try {
			await php.run({
				...
				$files = new RecursiveIteratorIterator(
					new RecursiveDirectoryIterator('/typo3')
				);
	...
	startServer({
		...
		onBind: async (port: number) => {
			...
			logger.log(`Setting up TYPO3 ${args.t3}`);
			let t3Details: any = undefined;
			...
			if (!args.skipWordPressSetup) {
				...
				monitor.addEventListener('progress', ((
					...
					if (!args.quiet) {
						...
						process.stdout.cursorTo(0);
						process.stdout.write(
							`Downloading TYPO3 ${percentProgress}%...`
						);
					}
				}) as any);

				t3Details = await resolveTypo3Release(args.t3);
			}
			logger.log(`Resolved TYPO3 release URL: ${t3Details?.releaseUrl}`);

			const preinstalledT3ContentPath =
				t3Details &&
				path.join(
					CACHE_FOLDER,
					`prebuilt-t3-content-for-wp-${t3Details.version}.zip`
				);
			const typo3Zip = !t3Details
				? undefined
				: fs.existsSync(preinstalledT3ContentPath)
				? readAsFile(preinstalledT3ContentPath)
				: await cachedDownload(
						t3Details.releaseUrl,
						`${t3Details.version}.zip`,
						monitor
				  );
			...
			logger.log(`Booting TYPO3...`);
			requestHandler = await bootTYPO3({
				siteUrl: absoluteUrl,
			...
			try {
				if (
					...
					logger.log(
						`Caching preinstalled TYPO3 for the next boot...`
					);
					fs.writeFileSync(
						preinstalledT3ContentPath,
						await zipDirectory(php, '/typo3-website')
					);
					logger.log(`Cached!`);
				}
				...
				if (command === 'build-snapshot') {
					...
					logger.log(`TYPO3 exported to ${args.outfile}`);
					process.exit(0);
				} else if (command === 'run-blueprint') {
					logger.log(`Blueprint executed`);
					process.exit(0);
				} else {
					logger.log(`TYPO3 is running on ${absoluteUrl}`);
				}
			} catch (error) {
				if (!args.debug) {
					throw error;
		...
		async handleRequest(request: PHPRequest) {
			if (!wordPressReady) {
				return PHPResponse.forHttpCode(502, 'TYPO3 is not ready yet');
			}
